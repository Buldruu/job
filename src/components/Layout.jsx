...
const nav = [
  {
    to: '/', label: 'Нүүр',
    icon: <svg className="w-4.5 h-4.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>,
  },
  {
    to: '/ajil', label: 'Ажил хайх',
...
export default function Layout() {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-cream-100">
      ...
      {/* ── Sidebar ── */}
      <aside className="w-60 flex-shrink-0 bg-white border-r border-cream-300 flex flex-col shadow-soft">

        {/* Brand */}
        <div className="px-5 py-5 border-b border-cream-200">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 bg-amber-500 rounded-xl flex items-center justify-center shadow-amber flex-shrink-0">
            ...
            <span className="text-lg font-display font-bold text-ink-900 tracking-tight">Haga</span>
          </div>
        </div>
        ...
        {/* Санхүү — тусдаа хэсэг */}
        <div className="px-3 pb-2 border-t border-cream-200 pt-3">
          <NavLink
            to="/sanhuu"
        ...
        {/* User */}
        <div className="px-3 pb-4 space-y-1 border-t border-cream-200 pt-3">
          <NavLink
            to="/profile"
            className={({ isActive }) => `sidebar-link ${isActive ? 'sidebar-link-active' : ''}`}
          >
            <div className="w-6 h-6 rounded-full overflow-hidden bg-amber-100 flex items-center justify-center flex-shrink-0">
              {photoURL
                ? <img src={photoURL} alt="" className="w-full h-full object-cover" />
                : <span className="text-xs font-bold text-amber-700">{displayName[0]?.toUpperCase()}</span>
              }
            </div>
          ...
          <button
            ...
            className="sidebar-link w-full text-rose-400 hover:text-rose-600 hover:bg-rose-50"
          >
            <svg className="w-4.5 h-4.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
