...
function ProfileChiglелSelect({ value, onChange }) {
  const [selMain, setSelMain] = useState(() => getMainCat(value) || '');
  const [selSub,  setSelSub]  = useState(() => getSubCat(value) || '');
  const handleMain = (m) => { setSelMain(m); setSelSub(''); onChange(''); };
  const handleSub  = (s) => { setSelSub(s); onChange(''); };
  return (
    <div className="space-y-2">
      <select value={selMain} onChange={e => handleMain(e.target.value)} className="input-base">
        <option value="">🗂 Том ангилал</option>
        {MAIN_CATS.map(m => <option key={m} value={m}>{m}</option>)}
      </select>
      {selMain && (
        <select value={selSub} onChange={e => handleSub(e.target.value)} className="input-base border-brand-300 bg-brand-50">
          <option value="">📂 Дэд ангилал</option>
          {getSubs(selMain).map(s => <option key={s} value={s}>{s}</option>)}
        </select>
      )}
      {selMain && selSub && (
        <select value={value} onChange={e => onChange(e.target.value)} className="input-base border-emerald-300 bg-emerald-50">
