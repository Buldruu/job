...
import { CHIGLEL_MAP, MAIN_CATS, getSubs, getProfs, getMainCat, getSubCat } from '../data/chiglel';
...
export default function JobList({ type }) {
  ...
  const filtered = items
    .filter(i => {
      ...
      if (filterProf) {
        if (i.chiglel !== filterProf) return false;
      } else if (filterSub) {
        // match sub OR any prof under it
        const profs = getProfs(filterMain, filterSub);
        if (i.chiglel !== filterSub && !profs.includes(i.chiglel)) return false;
      } else if (filterMain) {
        const subs = getSubs(filterMain);
        const allProfs = subs.flatMap(s => getProfs(filterMain, s));
        if (!subs.includes(i.chiglel) && !allProfs.includes(i.chiglel)) return false;
      }
  ...
  return (
    <div style={{padding:"16px",background:"var(--parchment)",minHeight:"100%"}}>
      ...
      <div className="space-y-3 mb-4 animate-fade-up-delay">
        ...
        {(filterMain || filterSub || filterProf || activeSearch) && (
          <div className="flex gap-2 flex-wrap items-center">
            ...
            {filterMain && (
              <span className="text-xs bg-surf-100 border border-surf-200 text-gray-600 px-2.5 py-1 rounded-full flex items-center gap-1">
                🗂 {filterMain}
                {!filterSub && <button onClick={() => { setFilterMain(''); setFilterSub(''); setFilterProf(''); }} className="hover:text-red-400 ml-1">✕</button>}
              </span>
            )}
            {filterSub && (
              <>
                ...
                <span className="text-xs bg-brand-50 border border-brand-200 text-brand-600 px-2.5 py-1 rounded-full flex items-center gap-1">
                  📂 {filterSub}
                  {!filterProf && <button onClick={() => { setFilterSub(''); setFilterProf(''); }} className="hover:text-red-400 ml-1">✕</button>}
                </span>
              </>
            )}
            {filterProf && (
              <>
                <span className="text-gray-300 text-xs">›</span>
                <span className="text-xs bg-emerald-50 border border-emerald-200 text-emerald-600 px-2.5 py-1 rounded-full flex items-center gap-1">
                  👤 {filterProf}
                  <button onClick={() => setFilterProf('')} className="hover:text-red-400 ml-1">✕</button>
                </span>
              </>
            )}
...
function ChiglелSelect({ value, onChange }) {
  // Determine initial selMain/selSub from existing value
  const [selMain, setSelMain] = useState(() => {
    if (!value) return '';
    return getMainCat(value) || '';
  });
  const [selSub, setSelSub] = useState(() => {
    if (!value) return '';
    return getSubCat(value) || '';
  });

  const handleMain = (m) => {
    setSelMain(m); setSelSub(''); onChange('');
  };
  const handleSub = (s) => {
    setSelSub(s); onChange('');
  };
  const handleProf = (p) => {
    onChange(p);
  };
  ...
  return (
    <div className="space-y-2">
      ...
      {/* Level 2 */}
      {selMain && (
        <select value={selSub} onChange={e => handleSub(e.target.value)}
          className="input-base border-brand-300 bg-brand-50">
          <option value="">📂 Дэд ангилал сонгоно уу</option>
          {getSubs(selMain).map(s => <option key={s} value={s}>{s}</option>)}
        </select>
      )}
      {/* Level 3 */}
      {selMain && selSub && (
        <select value={value} onChange={e => handleProf(e.target.value)}
          className="input-base border-emerald-300 bg-emerald-50">
          <option value="">👤 Мэргэжил сонгоно уу</option>
          {getProfs(selMain, selSub).map(p => <option key={p} value={p}>{p}</option>)}
        </select>
      )}
      {value && (
        <div className="text-xs text-emerald-700 bg-emerald-50 border border-emerald-200 px-3 py-1.5 rounded-xl inline-flex items-center gap-1.5">
          👤 {value}
          <button type="button" onClick={() => { onChange(''); setSelSub(''); setSelMain(''); }} className="hover:text-red-400 ml-1">✕</button>
        </div>
      )}
    </div>
  );
