...
const ERROR_MSGS = {
  ...
  'auth/email-already-in-use': 'Имэйл аль хэдийн бүртгэлтэй',
  'auth/weak-password': 'Нууц үг хэт богино байна (6+ тэмдэгт)',
...
/* ─── Shared background + logo wrapper ─── */
...
function SubmitBtn({ loading, label }) {
  return (
    <button
      type="submit"
      disabled={loading}
      className="w-full bg-brand-500 hover:bg-brand-600 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold py-3.5 rounded-xl transition-all shadow-lg shadow-brand-500/30 hover:shadow-brand-500/50 flex items-center justify-center gap-2"
    >
      {loading
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <label className="text-xs font-semibold text-white/50 uppercase tracking-wider">Нууц үг</label>
          <button
            type="button"
      ...
      <p className="text-center text-white/30 text-sm pt-1">
        Бүртгэл байхгүй юу?{' '}
        <button type="button" onClick={() => onSwitch('register')} className="text-brand-400 hover:text-brand-300 font-semibold transition">
          Бүртгүүлэх
        </button>
      </p>
    </form>
...
function RegisterForm({ onSwitch }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {error && <div className="bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-3 text-red-400 text-sm">{error}</div>}
      <SubmitBtn loading={loading} label="Бүртгүүлэх" />
      <p className="text-center text-white/30 text-sm pt-1">
        Аль хэдийн бүртгэлтэй юу?{' '}
        <button type="button" onClick={() => onSwitch('login')} className="text-brand-400 hover:text-brand-300 font-semibold transition">
          Нэвтрэх
        </button>
      </p>
    </form>
    ...
    <div className="space-y-5">
      ...
      {sent ? (
        <div className="bg-green-500/10 border border-green-500/20 rounded-2xl px-5 py-5 text-center">
          ...
          <p className="text-white/40 text-xs">{email} хаяг руу нууц үг сэргээх холбоос явуулсан. Имэйлээ шалгана уу.</p>
          <button
            type="button"
            onClick={() => onSwitch('login')}
...
export default function Login() {
  ...
  const titles = { login: 'Нэвтрэх', register: 'Бүртгүүлэх', forgot: 'Нууц үг сэргээх' };

  return (
    <AuthShell>
      <div className="glass rounded-2xl p-8">
        {/* Tab bar — only shown for login/register */}
        {screen !== 'forgot' && (
